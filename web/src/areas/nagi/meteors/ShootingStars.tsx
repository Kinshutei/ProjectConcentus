...
export function ShootingStars() {
  ...
  const handleIteration = useCallback((index: number, e: AnimationEvent<SVGSVGElement>) => {
    ...
    if (e.animationName !== 'nagi-star-flash') return
    setStars((prev) => {
      const next = [...prev]
  ...
  return (
    <div className="nagi-stars" aria-hidden="true">
      {stars.map((s, i) => {
        const w = s.len + PAD * 2
        return (
          <svg
